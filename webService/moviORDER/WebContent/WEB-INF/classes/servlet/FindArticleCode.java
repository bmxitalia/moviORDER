/*
 * Servlet che esegue la ricerca di un codice articolo dato il codice stesso (scritto dall'utente su un input type text), la path di connessione al db dell'azienda dove è contenuta la tabella Art e lo username dell'utente che ha accesso all'applicazione.
 ...
 * La Servlet esegue una seconda ricerca per verificare se il codice dell'articolo non è già presente nella tabella TmpRig, ovvero sell'utente non ha già l'articolo in carrello. In caso affermativo l'app visualizzerà un messaggio di notifica, in caso negativo verrà aperta la pagina per l'inserimento del nuovo prodotto in carrello.
 */
package servlet;
		...
		try {
			while(rs.next()) {
				if(rs.getInt(1)!=0) {
					...
					//se non è andata a buon fine la ricerca del codice articolo cerco il codice a barre
					rs.close();
